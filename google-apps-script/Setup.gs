function setupPachaDeportes() {
  ...
  createSheet_('Jugadores', ['playerId','teamId','teamName','fullName','dni','birthDate','categories','photoUrl','createdAt','firstName','lastName','documentType'], SEED_DATA.players.map(function(p){ return [p.playerId,p.teamId,p.teamName,p.fullName,p.dni,p.birthDate,p.categories,p.photoUrl,new Date(),'','','DNI']; }));
